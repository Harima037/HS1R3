            ...
            if(!puede_editar_avance){
                ...
                if(!response.data.cierre_fassa){
                    ...
                    if(response.data.idEstatusCierre){
                        ...
                        $('#avance-denominador').val(parseFloat(avance.denominadorCierre));
                        $('#avance-numerador').val(parseFloat(avance.numeradorCierre));
                        $('#avance-porcentaje').text(parseFloat(avance.porcentajeCierre).format(2) + ' %');
                        $('#justificacion').val(avance.justificacionAcumuladaCierre);
                        ...
                        if(avance.idEstatusCierre == 2 || avance.idEstatusCierre == 4 || avance.idEstatusCierre == 5){
                            bloquear_controles('.informacion-avance');
                        }else if(response.data.cierre_fassa == 2){
                            bloquear_controles('.informacion-avance');
                        }
                        ...
                        if(avance.idEstatusCierre == 4 || avance.idEstatusCierre == 5){
                            ...
                            $('#btn-imprimir-reporte').off('click');
                            $('#btn-imprimir-reporte').on('click',function(){
                                window.open(SERVER_HOST+'/v1/reporte-fassa/' + response.data.id + '?cierre=1');
                            });
                        }
                    }else if(response.data.cierre_fassa == 2){
                        bloquear_controles('.informacion-avance');
...
function actualizar_acciones_metas(){
    ...
    $('.informacion-meta').on('change',function(){
        ...
        if(trimestre == 4){
            $('#numerador-cierre').text((parseFloat(numerador) || 0).format(2))
            $('#denominador-cierre').text((parseFloat(denominador) || 0).format(2));
            $('#porcentaje-cierre').text((parseFloat(porcentaje) || 0).format(2));
        }
    });
}
