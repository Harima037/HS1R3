...
var moduloResource = new RESTfulRequests(SERVER_HOST+'/v1/reporte-seg-estrategia');
var moduloDatagrid = new Datagrid("#datagridProgramas",moduloResource,{ formatogrid:true, pagina: 1});
var moduloResourceFirmar = new RESTfulRequests(SERVER_HOST+'/v1/seguimiento-estrategia');
var firmar="";
var tmp="";
            ...
            if(response.data[i].evaluacion_trimestre){
                for(var j in response.data[i].evaluacion_trimestre){
                    ...
                    if(eval.idEstatus == 4){
                        ...
                        */
                        firmar='<li><a href="#" onClick="firmarEstrategia('+eval.trimestre+','+item.id+')" class="btn-edit-rows"><span class="glyphicon glyphicon-edit"></span> Firmar</a> </li>';
...
function cargarReporte(id,trimestre){
    ...
    window.open(SERVER_HOST+'/v1/reporte-seg-estrategia/'+parametros);
}
...
function firmarEstrategia(trim,proy){
   // alert(trim+" "+proy);
    var parametros = 'actualizarestrategia=firmar&trim_firma='+trim;   
    Confirm.show({
