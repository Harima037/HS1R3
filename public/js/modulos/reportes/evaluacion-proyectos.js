...
moduleDatagrid.actualizar({
    _success: function(response){
        ...
        for(var i in response.data){
            ...
            if(response.data[i].analisis_funcional.length > 0){
                item.cuentaPublica = '<span class="label label-info">Registrado</span>';
            }else{
                item.cuentaPublica = '<span class="text-muted">No Registrado</span>';
            }

            if(response.data[i].observaciones){
                item.observaciones = '<span class="label label-info">Registrado</span>';
            }else{
                item.observaciones = '<span class="text-muted">No Registrado</span>';
            }
...
function editar (e){
    ...
    moduleResource.get(e,parametros,{
        _success: function(response){
            ...
            $('#observaciones').val('');
            if(response.data.evaluacion_proyecto_observacion){
                $('#accion-observaciones').text('Editar');
                $('#id').val(response.data.evaluacion_proyecto_observacion.id);
                $('#observaciones').val(response.data.evaluacion_proyecto_observacion.observaciones);
...
$('#btnGuardarObservaciones').on('click',function(){
    var parametros = $('#formObservaciones').serialize()+'&mes='+$('#mes').val();
    if($('#id').val()){
